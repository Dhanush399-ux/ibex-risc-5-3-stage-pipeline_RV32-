...
module ibex_multdiv_slow (
  ...
  always @(multdiv_state_q, accum_window_q, op_a_shift_q, op_b_shift_q, op_numerator_q, md_state_q, 
           op_a_bw_last_pp, op_a_i, op_b_i, op_b_shift_q) begin
    alu_operand_a_o   = accum_window_q;
    multdiv_result_o  = div_en_i ? accum_window_q[31:0] : res_adder_l;

    case(operator_i)

      ibex_pkg::MD_OP_MULL: begin
        alu_operand_b_o   = op_a_bw_pp;
      end

      ibex_pkg::MD_OP_MULH: begin
  ...
  always @(multdiv_state_q, accum_window_q, op_a_shift_q, op_b_shift_q, op_numerator_q, md_state_q, 
           mult_en_i, div_en_i, op_a_ext, op_b_i, op_b_ext, equal_to_zero, sign_a, alu_adder_i, op_a_i,
           sign_b, res_adder_l, next_reminder, next_quotient, div_change_sign) begin
    ...
    if (mult_en_i || div_en_i) begin
      case(md_state_q)
        ...
        default: begin
          md_state_d = MD_IDLE; // Use a proper enum value
        end
        endcase // md_state_q
      end
  end
  ...
  assign valid_o = (md_state_q == MD_FINISH) |
                   (md_state_q == MD_LAST &
                   (operator_i == ibex_pkg::MD_OP_MULL |
                    operator_i == ibex_pkg::MD_OP_MULH));
